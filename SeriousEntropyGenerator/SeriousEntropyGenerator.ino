...
void loop(void)
{
 const int pins[]  = {A8, A9, A10, A11, A12, A13, A14, A15};
 while(true)
 {
  ...
  for(int i = 0; i < 8;)
  {
   const auto value  = analogRead(pins[i]);
   if(value == 0)
   {
    continue;
   }
   else
   {
    ...
    {
     const auto ip_address  = client.remoteIP();
     const auto ip_address_numeric  = ip_address[0] | (ip_address[1] << 8) | (ip_address[1] << 16) | (ip_address[3] << 24);
